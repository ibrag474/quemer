...
class restoreForm extends React.Component {
	constructor() {
		super();
		this.state = {code: '', password: '', repassword: '', btnStatus: false, message: ''};
	...
	handleSubmit(e) {
		...
		submitPswdReset(this, toSend.code, toSend.password, toSend.repassword, (context, status, result) => {
			if (status == 200) {
				context.setState({btnStatus: true, message: result.message + ' Now you can' + cel('a', {href: '/account/login', key: 'loginA'}, 'login')});
	...
	render() {
		...
		return cel('form', {key: 'res-form', onSubmit: this.handleSubmit}, [
			...
			cel('input', {type: 'submit', value: 'Submit', disabled: btnStatus}, null),
		]);
	}
}
